...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition=="RA" and len(self.stack)>1):
            a = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1],a))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(len(unfinished_parses)>0):
        parse_batch = unfinished_parses[0:batch_size]
        transitions = model.predict(parse_batch)
        for t,p in zip(transitions,parse_batch):    #Apply one transition to each partial parse
            p.parse_step(t)
